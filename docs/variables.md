...
**Reserved tokens are left alone.** A token opening with `>`, `#` or `!` — such
as `{{> Component Name}}` — is not a variable, and passes through substitution
unchanged. These are held for future syntax.
