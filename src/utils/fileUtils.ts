...
export const parseLoadedData = (
  ...
  if (typeof uuidv4 !== 'function') {
    console.error(
      "[fileUtils|parseLoadedData] uuidv4 is not a function. Please ensure the 'uuid' package is installed and imported correctly."
    );
  ...
  try {
    ...
    if (
      data &&
      typeof data === 'object' &&
      (data.tree !== undefined || data.prompts !== undefined) &&
      !Array.isArray(data)
    ) {
    ...
    if (!Array.isArray(treeToProcess)) {
      ...
      console.error(
        '[fileUtils|parseLoadedData] Tree data is not an array after initial parsing. Tree received:',
        treeToProcess
      );
    ...
    if (!Array.isArray(promptsToProcess)) {
      ...
      console.warn(
        '[fileUtils|parseLoadedData] Prompts data is not an array after initial parsing, defaulting to empty. Prompts received:',
        promptsToProcess
      );
      promptsToProcess = [];
    }
    ...
    const processNode = (node: any, parentPath: string = 'ROOT'): FolderType | ComponentType => {
      ...
      if (!node || typeof node !== 'object' || !node.type || typeof node.name !== 'string') {
        // Stricter name check
        console.error(
          `[fileUtils|processNode] Invalid node structure at path ${currentPath}. Node data:`,
          node
        );
        throw new Error(
          `Invalid node structure at path ${currentPath}: Missing type, name, or node is not an object.`
        );
      }
      ...
      if (typeof newId !== 'string' || newId.length === 0) {
        console.error(
          `[fileUtils|processNode] CRITICAL: uuidv4() returned an invalid ID: '${newId}' for node "${node.name}" at path ${currentPath}`
        );
      ...
      if (node.type === 'folder') {
        if (!Array.isArray(node.children)) {
          console.warn(
            `[fileUtils|processNode] Folder "${node.name}" at path ${currentPath} missing children array, defaulting to empty.`
          );
          node.children = [];
        }
        processedChildNode = {
        ...
        if (typeof processedChildNode.id !== 'string' || processedChildNode.id.length === 0) {
          console.error(
            `[fileUtils|processNode] CRITICAL: Folder "${processedChildNode.name}" at path ${currentPath} is being returned with an invalid ID: ${processedChildNode.id}`
          );
        }
        ...
        if (typeof content !== 'string') {
          console.warn(
            `[fileUtils|processNode] Component "${node.name}" at path ${currentPath} missing or invalid content, defaulting to empty string. Content was:`,
            content
          );
          content = '';
        }
        ...
        if (!validComponentTypes.includes(componentType)) {
          console.warn(
            `[fileUtils|processNode] Invalid componentType "${componentType}" for component "${node.name}" at path ${currentPath}. Defaulting to "context".`
          );
        ...
        if (typeof processedChildNode.id !== 'string' || processedChildNode.id.length === 0) {
          console.error(
            `[fileUtils|processNode] CRITICAL: Component "${processedChildNode.name}" at path ${currentPath} is being returned with an invalid ID: ${processedChildNode.id}`
          );
        }
      } else {
        console.error(
          `[fileUtils|processNode] Unknown node type: "${node.type}" for node "${node.name}" at path ${currentPath}`
        );
        throw new Error(
          `Unknown node type: "${node.type}" for node "${node.name}" at path ${currentPath}`
        );
      }
    ...
    const processedTree: FolderType[] = treeToProcess.map(
      (node: any) => processNode(node) as FolderType
    );
    ...
    const processedPrompts = promptsToProcess.map((prompt: any): Prompt => {
      if (
        ...
        console.error(
          '[fileUtils|parseLoadedData] Invalid prompt structure during mapping:',
          prompt
        );
      ...
      if (typeof newPromptId !== 'string' || newPromptId.length === 0) {
        console.error(
          `[fileUtils|parseLoadedData] CRITICAL: uuidv4() returned an invalid ID for prompt "${prompt.name}"`
        );
      ...
      return {
        ...
        sections: prompt.sections.map((section: any): Section => {
          if (
            ...
            console.error(
              '[fileUtils|parseLoadedData] Invalid section structure during mapping:',
              section
            );
          ...
          if (typeof newSectionId !== 'string' || newSectionId.length === 0) {
            console.error(
              `[fileUtils|parseLoadedData] CRITICAL: uuidv4() returned an invalid ID for section "${section.name}" in prompt "${prompt.name}"`
            );
          ...
          if (typeof section.linkedComponentId === 'number') {
            newLinkedComponentId = oldNumericIdToNewStringIdMap.get(
              section.linkedComponentId as number
            );
          } else if (
            typeof section.linkedComponentId === 'string' &&
            section.linkedComponentId.length > 0
          ) {
          ...
          return {
            ...
            editingHeaderTempType: validSectionTypes.includes(section.editingHeaderTempType)
    ...
    console.error(
      '[fileUtils|parseLoadedData] Error during parsing function. Error message:',
      error.message,
      'Stack:',
      error.stack,
      'Timestamp:',
