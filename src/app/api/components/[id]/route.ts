...
export async function PUT(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const { name, parent_id, item_type, content, component_type, expanded } = body as Partial<
      FolderType & ComponentType & { expanded: boolean }
    >;
    ...
    if (content !== undefined) {
      updates.push({
        column: 'content',
        value: effectiveItemType === 'component' ? content : null,
      });
    }
    ...
    return updated
      ...
      : errorResponse('Failed to retrieve updated item', 500);
  } catch (error) {
