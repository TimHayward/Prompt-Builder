...
export const usePrompts = () => {
  const { prompts, activePromptId, addSectionToPrompt, updateSection } = usePromptContext();
   ...
   */
  const createSectionFromComponent = useCallback(
    (promptId: string, componentId: string) => {
   ...
   */
  const sectionNeedsUpdate = useCallback(
   ...
   */
  const saveSectionToComponentLibrary = useCallback(
    (promptId: string, sectionId: string) => {
      ...
      if (!section) {
        console.error(
          `saveSectionToComponentLibrary: Section with id ${sectionId} not found in prompt ${promptId}.`
        );
        return;
      }
      ...
      if (!section.linkedComponentId) {
        console.warn(
          `saveSectionToComponentLibrary: Section ${sectionId} is not linked to a component.`
        );
        return;
      }
      ...
      if (!componentToUpdate) {
        console.error(
          `saveSectionToComponentLibrary: Linked component with id ${section.linkedComponentId} not found in the tree.`
        );
